...
def tabla_perdidas_dinamica(cap0, pct, p_base, r, L=5, filas=10):
    ...
    for i in range(1, filas + 1):
        ...
        rows.append((
            ...
            f"{p_cond*100:.2f} %", f"{(1-p_cond)*100:.2f} %",
            f"{(1-p_rn)*100:.2f} %", f"{p_rn*100:.2f} %"
        ))
...
resumen_box = Output(layout={'width': '370px'})
tabla_box   = Output(layout={'width': '730px'})
chart_box   = Output()

ui_simulador = HBox([resumen_box, tabla_box])
...
def simulador_kelly(P, R, kelly_fraccion, capital_inicial,
    ...
    with resumen_box:
        resumen_box.clear_output()
        print("Simulación Interactiva de Kelly Mejorado")
...
__all__ = [
    ...
    "ui_simulador", "chart_box"
]

if __name__ == "__main__" or "get_ipython" in globals():
    display(sliders_box, ui_simulador, chart_box, heatmap_output)
